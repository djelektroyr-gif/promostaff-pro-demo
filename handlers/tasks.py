...
async def task_description_received(message: types.Message, state: FSMContext):
    ...
    for a in assignments:
        keyboard_rows.append([InlineKeyboardButton(text=a[5], callback_data=f"assign_task_{a[1]}")])
...
async def show_my_tasks_for_shift(callback: types.CallbackQuery):
    ...
    for t in tasks:
        ...
        if t[6] != 'completed':
            keyboard_rows.append([InlineKeyboardButton(text=f"📝 {t[2][:20]}", callback_data=f"complete_task_{t[0]}")])
...
async def complete_task_start(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("📝 *ОТЧЁТ*\n\nОпишите, что сделано (или `-`):", parse_mode="Markdown")
