...
async def show_my_shifts(callback: types.CallbackQuery):
    ...
    if client:
        ...
        cur.execute("""
            ...
            WHERE p.client_id = ? ORDER BY s.shift_date DESC
        """, (user_id,))
        shifts = cur.fetchall()
        conn.close()
        ...
        for s in shifts:
            ...
            keyboard_rows.append([InlineKeyboardButton(
                text=f"{s[1]} {s[2]}-{s[3]}", callback_data=f"shift_detail_{s[0]}"
            )])
        ...
        cur.execute("""
            ...
            FROM assignments a JOIN shifts s ON a.shift_id = s.id
            WHERE a.worker_id = ? ORDER BY s.shift_date DESC
        """, (user_id,))
        shifts = cur.fetchall()
        conn.close()
        ...
        for s in shifts:
            ...
            keyboard_rows.append([InlineKeyboardButton(
                text=f"{s[1]} {s[2]}-{s[3]}", callback_data=f"worker_shift_{s[0]}"
            )])
        keyboard_rows.append([InlineKeyboardButton(text="🔙 Назад", callback_data="main_menu")])
        keyboard = InlineKeyboardMarkup(inline_keyboard=keyboard_rows)
        await callback.message.edit_text(text, reply_markup=keyboard, parse_mode="Markdown")
...
async def checkout_geo_received(message: types.Message, state: FSMContext):
    ...
    await message.answer("📸 *ЧЕК-АУТ*\n\nОтправьте финальное фото (или `0` чтобы пропустить).", parse_mode="Markdown")
...
async def show_shift_report(callback: types.CallbackQuery):
    ...
    shift = report['shift']
    text = f"📊 *ОТЧЁТ ПО СМЕНЕ #{shift_id}*\n\n📆 {shift[2]} | {shift[3]}-{shift[4]}\n📍 {shift[5]}\n💰 Ставка: {shift[6]} ₽/час\n\n👥 *ИСПОЛНИТЕЛИ:*\n"
    total_payment = 0
